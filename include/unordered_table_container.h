...
public:
    ...
    public:
        orderedTableIteratorAdapter(UnorderedTable<std::string, Polynom>::Iterator& iter)
            : it(iter) {
        }
        ...
        std::pair<std::string, Polynom>& operator++(int) override {
            UnorderedTable<std::string, Polynom>::Iterator temp = it;
            it++;
            return *temp;
        }
    ...
    polynomContainer::Iterator* begin() override {
        UnorderedTable<std::string, Polynom>::Iterator it = data.begin();
        return new orderedTableIteratorAdapter(it);
    }

    polynomContainer::Iterator* end() override {
        UnorderedTable<std::string, Polynom>::Iterator it = data.end();
        return new orderedTableIteratorAdapter(it);
    }
